...
function setupSheet() {
  ...
  // CitizenID: luôn là TEXT để không mất số 0 đầu.
  sh.getRange('I:I').setNumberFormat('@');

  // UpdatedAt: là DATE thật, hiển thị ngày + giờ.
...
function doPost(e) {
  ...
  try {
    ...
    if (employees.length > 0) {
      ...
      // ------------------------------------------------------
      // FORMAT TRƯỚC KHI GHI
      // I = CitizenID -> TEXT
      // J = UpdatedAt -> DATE
      // ------------------------------------------------------
      sheet.getRange(2, 9, values.length, 1)
        .setNumberFormat('@');
      ...
      // ------------------------------------------------------
      sheet
        .getRange(2, 1, values.length, 22)
        .setValues(values);

      // ------------------------------------------------------
      // Format B:E và K:V
      // ------------------------------------------------------
      sheet.getRange(2, 2, values.length, 4)
        .setNumberFormat('0.000');

      sheet.getRange(2, 11, values.length, 12)
        .setNumberFormat('0.000');

      // Nhắc lại format sau khi ghi để Sheets không tự đổi kiểu.
      sheet.getRange(2, 9, values.length, 1)
        .setNumberFormat('@');
...
function doGet(e) {
  ...
  try {
    ...
    // --------------------------------------------------------
    // Hỗ trợ nhiều kiểu tham số để không phá frontend cũ:
    // ?name=Nguyen Van A
    // ?name=089196005003
    // ?citizenId=089196005003
    // ?cccd=089196005003
    // ?q=089196005003
    // --------------------------------------------------------
    ...
    // Nếu giá trị tìm kiếm chỉ gồm số thì coi là CCCD.
    const isCitizenQuery = /^\d[\d\s'.]*$/.test(rawQuery);

    const citizenQuery = normalizeCitizenId(
      isCitizenQuery ? rawQuery : rawCitizenId
    );

    const nameQuery = isCitizenQuery
      ? ''
      : normalizeVietnamese(rawQuery);

    // --------------------------------------------------------
    // Health check
    // --------------------------------------------------------
    if (!rawQuery && !rawCitizenId) {
    ...
    if (citizenQuery) {
      ...
      for (let i = 1; i < values.length; i++) {
        ...
        const dbCitizenId =
          normalizeCitizenId(values[i][8]);

        if (
          dbCitizenId &&
          dbCitizenId === citizenQuery
        ) {
    ...
    if (nameQuery) {
      ...
      for (let i = 1; i < values.length; i++) {
        ...
        if (
          normalizeVietnamese(dbName) === nameQuery
        ) {
      ...
      for (let i = 1; i < values.length; i++) {
        ...
        if (
          normalizedDbName.includes(nameQuery)
        ) {
    ...
    return jsonResponse({
      ok: false,
      error: isCitizenQuery
        ? 'Không tìm thấy nhân viên theo căn cước'
        : 'Employee not found'
    });
 ...
 * ============================================================
 *
 * CitizenID/CCCD luôn có 12 chữ số.
 * - Loại dấu ' nếu nguồn Excel có thêm dấu nháy.
 * - Loại khoảng trắng.
 * - Loại phần thập phân kiểu 89196005003.000.
 * - Nếu còn 11 chữ số thì tự thêm 0 ở đầu.
 * - Trả về STRING để Google Sheets không chuyển thành NUMBER.
 * ============================================================
 */
function normalizeCitizenId(value) {
  ...
  // Excel/DSCNV có thể có dấu ' ở đầu.
  s = s.replace(/^'+/, '');

  // Bỏ khoảng trắng.
  s = s.replace(/\s+/g, '');

  // Dữ liệu kiểu 12345678901.000
  if (/^\d+\.0+$/.test(s)) {
    s = s.split('.')[0];
  }

  // Dữ liệu kiểu scientific notation.
  if (/^[+-]?\d+(?:\.\d+)?[eE][+-]?\d+$/.test(s)) {
    const n = Number(s);
  ...
  // Chỉ nhận chuỗi số sau khi làm sạch.
  if (!/^\d+$/.test(s)) {
    return '';
  }
  ...
  if (s.length < 12) {
    ...
    // Không cắt dữ liệu bất thường; trả nguyên để dễ phát hiện.
    return s;
  }
