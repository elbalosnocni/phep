...
Private Sub ReadEmployeeMaster(ByVal masterDict As Object)
    ...
    For r = FIRST_DATA_ROW To lastRow
        ...
        If Len(employeeKey) > 0 Then
            ...
            ' CitizenID đã được chuẩn hóa thành TEXT 12 số.
            ' Ví dụ 89196005003 -> 089196005003.
...
Private Function ReadCitizenId(ByVal cell As Range) As String
    ...
    ' ========================================================
    ' CCCD luôn 12 chữ số.
    ' Ưu tiên .Text vì nếu ô DSCNV đã là Text hoặc có dấu '
    ' thì Excel sẽ giữ nguyên số 0 đầu.
    ' ========================================================
    On Error Resume Next
    ...
    ' Excel có thể hiển thị số kiểu 89196005003.000
    If InStr(s, ".") > 0 Then
        If IsNumeric(s) Then
            d = CDbl(s)
            s = Format$(d, "0")
    ...
    ' --------------------------------------------------------
    ' Fallback: đọc Value2.
    ' Nếu ô là NUMBER, Format 12 số để khôi phục số 0 đầu.
    ' --------------------------------------------------------
    v = cell.Value2
